...
int main(int argc, char **argv) {
    ...
    Sophus::SE3 T_cur_ref;//(Eigen::Matrix3d::Identity(),Eigen::Vector3d::Zero());
    ...
    for (int iter = 0; iter < iterations; iter++) 
    {
        ...
        for (size_t i = 0; i < px_ref.size(); i++) 
        {
            ...
            if(u-4<0 || u+4>=img1.cols || v-4 <0 || v+4>=img1.rows ||
               u2_opt-4<0 || u2_opt+4>=img2.cols || v2_opt -4<0 || v2_opt>=img2.rows)
            {
                continue;
            }
...
void DirectPoseEstimationMultiLayer(
    ...
    for(int i = 0;i<pyramids;i++)
    {
        ...
        cv::resize(img1, img1_temp, cv::Size(img1.cols*scales[i],img2.rows*scales[i]) );
